...
const DEFAULT_UI = {
  title: "RSA 密钥生成器",
  subtitle: "生成 RSA-OAEP 密钥对并导出 PEM（纯本地）",
  bitsLabel: "位数",
  generateButton: "生成密钥",
  generating: "生成中…",
  clearButton: "清空",
  ...
  publicKeyTitle: "公钥（SPKI PEM）",
  privateKeyTitle: "私钥（PKCS8 PEM）",
  copyButton: "复制",
  downloadButton: "下载",
  outputPlaceholder: "点击“生成密钥”后显示…",
  tip: "提示：密钥在浏览器本地生成；私钥请妥善保存，不要在不可信环境中生成与使用。",
} as const;

type RsaKeyGeneratorUi = typeof DEFAULT_UI;
...
export default function RsaKeyGeneratorClient() {
  const config = useOptionalToolConfig("rsa-key-generator");
  const ui: RsaKeyGeneratorUi = { ...DEFAULT_UI, ...((config?.ui ?? {}) as Partial<RsaKeyGeneratorUi>) };
  ...
  const generate = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : ui.generationFailed);
    } finally {
      setIsWorking(false);
    }
  };
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
            <div>
              <div className="mb-2 flex items-center justify-between gap-2">
                <div className="text-sm font-semibold text-slate-900">{ui.publicKeyTitle}</div>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    disabled={!publicPem}
            ...
            <div>
              <div className="mb-2 flex items-center justify-between gap-2">
                <div className="text-sm font-semibold text-slate-900">{ui.privateKeyTitle}</div>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    disabled={!privatePem}
