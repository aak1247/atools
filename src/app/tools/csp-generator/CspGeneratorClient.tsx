...
// 中文默认值
const DEFAULT_UI = {
  title: "内容安全策略 (CSP) 配置工具",
  description: "Content-Security-Policy 用于限制资源加载与脚本执行，提升 XSS 防护。建议配合 `nonce-`/`sha256-` 等安全策略。",
  generatorTitle: "生成器",
  parserTitle: "解析/校验",
  resetButton: "重置",
  copyButton: "复制 CSP",
  tipsTitle: "提示/风险点",
  directiveListTitle: "指令列表",
  addDirectiveButton: "添加指令",
  placeholderValue: "例如：'self' https: data: 'nonce-...'",
  deleteButton: "删除",
  cspOutputTitle: "CSP 输出",
  fillInParser: "填入解析器",
  commonSources: "常用源：",
  inputCspHeaderTitle: "输入 CSP Header",
  unknownDirectives: "未识别的指令：",
  applyToGenerator: "应用到生成器",
  parseResultsTitle: "解析结果",
  noParseResults: "暂无解析结果",
  normalizedOutput: "标准化输出（用于 header）：",
  warningUnsafeInlineScript: "script-src 包含 'unsafe-inline'：会显著降低 XSS 防护能力（建议 nonce/hash）。",
  warningUnsafeInlineStyle: "style-src 包含 'unsafe-inline'：建议使用 nonce/hash 或尽量避免内联样式。",
  warningUnsafeEval: "script-src 包含 'unsafe-eval'：会允许 eval/new Function 等，风险较高。",
  warningWildcard: "存在 * 源：建议仅放行必要源，避免过度授权。",
  warningObjectSrc: "建议将 object-src 设为 'none'（除非必须）。",
...
function CspGeneratorInner() {
  ...
  const buildWarnings = (directives: DirectiveLike[]) => {
    ...
    if (hasUnsafeInlineScript) warnings.push(ui.warningUnsafeInlineScript);
    if (hasUnsafeInlineStyle) warnings.push(ui.warningUnsafeInlineStyle);
    if (hasUnsafeEval) warnings.push(ui.warningUnsafeEval);
    if (hasWildcard) warnings.push(ui.warningWildcard);

    const objectSrc = map.get("object-src");
    if (objectSrc && !objectSrc.includes("'none'")) warnings.push(ui.warningObjectSrc);

    if (!map.has("base-uri")) warnings.push(ui.warningBaseUri);
    if (!map.has("frame-ancestors")) warnings.push(ui.warningFrameAncestors);
      ...
      <div className="glass-card rounded-3xl p-6 shadow-2xl ring-1 ring-black/5">
        ...
        {warnings.length > 0 && (
          <div className="mt-4 rounded-2xl bg-amber-50 px-4 py-3 text-sm text-amber-900 ring-1 ring-amber-100">
            <div className="font-semibold">{ui.tipsTitle}</div>
        ...
        {mode === "builder" ? (
                  ...
                  <div key={`${d.name}-${idx}`} className="rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-200">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <input
                        ...
                        placeholder={ui.placeholderValue}
                      />
                      <button
                        type="button"
                    ...
                    <div className="mt-2 text-[11px] text-slate-500">{ui.tokensLabel}{splitTokens(d.value).join(" · ") || "-"}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="rounded-3xl bg-white p-5 ring-1 ring-slate-200">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-sm font-semibold text-slate-900">{ui.cspOutputTitle}</div>
                  <button
                    type="button"
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
            <div className="rounded-3xl bg-white p-5 ring-1 ring-slate-200">
              <div className="text-sm font-semibold text-slate-900">{ui.inputCspHeaderTitle}</div>
              <textarea
                value={rawHeader}
              ...
              {parsed.unknown.length > 0 && (
                <div className="mt-3 text-xs text-amber-800">
                  {ui.unknownDirectives}{parsed.unknown.join(", ")}
                </div>
              )}
            ...
            <div className="rounded-3xl bg-white p-5 ring-1 ring-slate-200">
              <div className="text-sm font-semibold text-slate-900">{ui.parseResultsTitle}</div>
              <div className="mt-3 max-h-64 overflow-auto rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-200">
                {parsed.directives.length === 0 ? (
                  <div className="text-xs text-slate-500">{ui.noParseResults}</div>
                ) : (
              ...
              <div className="mt-3 text-xs text-slate-500">
                {ui.normalizedOutput}<span className="font-mono break-words">{normalizeHeader(parsed.directives)}</span>
              </div>
            </div>
          </div>
        )}
